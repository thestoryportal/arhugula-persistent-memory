...
- **MIXED-LOAD SMOKE (done 2026-06-21 — pure-capital k≤2 does NOT survive realistic load):** held-out *capital* corruption, pure (k capital) vs mixed (k capital + 12 language edits on disjoint entities), 12 randomized orders, lang-expr 100%:

  | cap_k | pure% | mixed% | mixed-UCB% | Δ cross-rel | worst% |
  |---|---|---|---|---|---|
  | 1 | 0.0 | 2.08 | 3.98 | +2.08pp | 8.3 |
  | 2 | 0.0 | 2.43 | 4.42 | +2.43pp | 12.5 |
  | 3 | 0.0 | 3.82 | 6.15 | +3.82pp | 25.0 |

  **Mixed-load clean ceiling = k=0** (vs pure-capital k=2). Held-out capital is corrupted at *every* capital-k≥1 once other-relation load is present; at k=1 the capital contribution is ~0, so the corruption is essentially **12 language edits alone** perturbing the shared subspace, with capital-k adding an interaction on top (Δ grows 2.08→3.82pp). **This VINDICATES the §3 worse-of(global-edge-count, per-relation-concentration) design** — the global/total-volume tier is independently necessary because other-relation *volume* corrupts a relation's reads; per-relation count alone is a sentinel, not sufficient. Magnitude modest (2–4% at k=1–3, UCB crosses 5% by k=3) — a bounded cross-relation floor. **Deployment implication: under realistic mixed load the per-relation guardrail must be paired with a global-volume bound + frequent compaction; `k≤2` per-relation is necessary but not sufficient alone.** Artifact `results/d1_mixedload_smoke_3b_s3.json`.
- **Recommended next (not blocking; same direction):** repeat low-k with the more-toxic held-out seed; ≥3 held-out seeds; vary the mixed other-relation load magnitude; cluster bootstrap already done. Artifacts: `results/d1_threshold_lowk_3b_s3{,_lowextra}.json`, `results/d1_threshold_curve_3b_*.json`, `results/d1_instrument_variance_diagnostic_3b_{nondet,det}.json`; reviews `logs/codex_review_threshold_{,FINAL_}OUT.log`.
