...
2. **B1 (reframed) — ✅ RESOLVED: NOT RUN (operator-accepted, 2026-06-22).** Advisor+Codex next-step review + the spec-reading discriminator (no clause requires bare weights to enforce closed-world reads) → B0's leak is the **spec's design, not a falsifier** (commit-status delegated to State Ledger/§26). **B0 folded as the obligation-medium disposition** (read-time closed-world = `GOVERNANCE_MAY_ENFORCE`; spec under-specifies the read-provenance surface — F1 finding + spec gap; matrix R6). B1's only spec-falsifying half (committed-fact reliability under **scale + quantization**) is **routed into the compaction-CORE-at-scale run** as emitted seam metrics, with **factorial scale-only / quant-only / combined** (Codex). The cheap-confidence-gate membership-predicate test = an *optimization* (spec provides the ledger) → parked.
3. **Read/query matrix extraction — ✅ FIRST SLICE DONE (2026-06-22):** `docs/READ_QUERY_CONTRACT_MATRIX.md` — 16 read requirements spec-first-extracted, tagged evidence-type · Medium-of-Obligation · fail-criterion · coverage. Surfaced the WEIGHTS-owed falsifiers (R5 native-knowing-at-scale, R2 reverse-lookup, R9 deletion-non-resurfacing, R15 constraint-probe) vs the GOV/HYBRID-delegated reads, + spec gaps (R6/R7/R8/R13). Other-layer slices follow the same schema.
4. **Stimulus pool V0/V1** (operator-approved infra) — serves the compaction-at-scale run + the WEIGHTS falsifiers: large-N + CORE/SUPPORTING/INCIDENTAL tags + reverse-pairs + 5-relation-family + (for R5) deliberately high-confidence uncommitted-known negatives.
