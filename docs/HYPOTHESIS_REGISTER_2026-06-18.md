...
|---|---|---|---|---|
| **D20** | **Compaction-at-scale may REINTRODUCE the corruption it heals.** The §8.10 self-heal assumes "compaction = full MEMIT re-run → returns to clean A1." But A1 validated cleanliness only at **N≤100 / 3B single-batch**, while §8.7 lets drift reach **1,500–8,000 edges** before compaction and **MEMIT batch size is 2,000** (spec line 384) → above that, compaction is **sub-batched = multiple sequential solves** = the exact G6.1/D-D1-2 sequential-accumulation corruption mechanism. | **B3N condition 3** / §8.10 / §10.4 / builds-on A1 (`D-A1-1`) + the G6.1 sequential-corruption finding + D1 (`D-D1-2`) | **The sharpest open F1 falsifier.** If compaction isn't clean at scale, B3N condition 2 ("compact in time") cannot save the in-weight verdict — there is no clean state to return to, and the hybrid needs the side-store after all. Higher-priority than the 7B numeric transfer. | **DONE = WEAK-FORM FALSIFICATION (D-D20-1, CORPUS/23).** Component 2 (chunking): partitioning a clean N=100 joint solve into accumulating sub-batches reintroduces held-out corruption, **ROBUST by C=10** (10 sub-batches: −19 to −34pp across 3 edit orderings; expr 96-100%; anchor gates pass). **C=25 (4 sub-batches) is ORDER-SOFT → NOT promotable** (robust floor = C=10). → the spec's sub-batched compaction cannot be assumed to return to clean = **B3N condition 3 EVIDENCED-FAILING in its chunking component.** **BOUND: falsifies but doesn't confirm** (N held at known-clean 100 → isolates chunking, not scale). **Component 1 (SCALE) still OPEN** — single-solve cleanliness at true drift size + where the spec's 2,000-SIZE boundary sits vs the C=10 floor; gated on a larger stimulus pool. NOT a promoted node. Dual-reviewed (advisor ×2 + gpt-5.5). `docs/D20_COMPACTION_SUBBATCH_PREREG.md`. |
