...
**Axis B finding (spec-grounded):** In-weight is the spec's substrate for **both** genesis and incremental overlays. It is **viable for the spec's compaction-bounded hybrid CONDITIONAL on**: (a) the drift guardrail being **concentration-aware** (our amendment), not count-only; (b) **compaction running before the clean envelope is exceeded;** and **(c) compaction's own (sub-batched) re-run staying clean at the realized drift size — D-D20-1: DIRECTIONALLY PRESSURED, not falsified (§6 ⚠).** The routed/gated side-store becomes the stronger candidate **if real incremental churn is high enough that compaction cannot keep each inter-anchor window clean, *or* if condition (c) fails** (compaction itself reintroduces corruption at scale) — i.e., genuine high-churn-online editing, a *more demanding* profile than the spec's bounded-then-compacted overlays.
...
> **In-weight is the viable substrate for the spec's compaction-bounded hybrid, and B3 does NOT falsify the spec's in-weight thesis — CONDITIONAL on THREE guardrails:** (1) the §8.7 drift trigger must be **concentration-aware** (our D-D1-2 amendment, `k≤1`), not count-only — this keeps each inter-compaction window inside the clean envelope; (2) **compaction must run before that envelope is exceeded;** and **(3) compaction's own (sub-batched) re-run must STAY CLEAN at the realized drift size** — **DIRECTIONALLY tested by D-D20-1 (CORPUS/23): pressured, NOT falsified** — accumulating sub-batching can reintroduce corruption even when the joint solve is clean, but the K-vs-C confound + spec-2000-SIZE regime are open (see ⚠ below). A routed/gated **side-store is the stronger candidate only for genuine high-churn-online editing** — a profile *beyond* the spec's bounded-then-compacted overlays — and even then conditional on an unproven NL→query routing layer (§4).

> **⚠ CONDITION 3 — the load-bearing untested assumption (advisor reconcile pass, 2026-06-21).** The whole self-heal story rests on "§8.10 compaction = a full MEMIT re-run → returns to the clean A1 regime." But A1 validated batch-cleanliness **only at N≤100 / 3B single-batch**, while: **(a)** §8.7 lets drift reach **1,500–8,000 edges** before compaction fires — 15–80× beyond A1's tested scope (and B1/D-B1-1 already showed the batch path wobble to 91.7% at 7B); **(b)** the **MEMIT batch size is 2,000** (spec line 384) — a realistic-drift compaction is **sub-batched into multiple sequential solves**, which is *exactly* the sequential-accumulation mechanism G6.1/D-D1-2 found corrupting. So **compaction at scale may reintroduce the corruption it is meant to heal.** "Returns to clean" is currently an *extrapolation*, not a result. If condition 3 fails, condition 2 ("compact in time") does **not** save the verdict — there is no clean state to return to, and the in-weight hybrid would need the side-store after all. **Discriminating test (next-arc): does the batch/compaction path stay clean as it sub-batches / at scale?** A1 cannot answer this (single-batch ≤100). This connects the overlay's §8.10 row with its §10.4 row.

> **✅ UPDATE (D-D20-1, 2026-06-21 — `CORPUS/23`):** condition 3 is now **DIRECTIONALLY TESTED, not falsified.** Finding: the engine does a *single joint solve regardless of N* (so the "2,000" is a spec *prescription*, not engine behavior), and **accumulating sequential sub-batching can reintroduce held-out corruption even when the equivalent joint solve is clean** — order-insensitive at ~10 sub-batches. So condition 3 is **directionally PRESSURED (compaction can't be *naively* assumed clean once it sub-batches)**, but **NOT falsified:** two gaps remain — **(a) the K-vs-C confound** (at fixed N, chunk-size couples with chunk-count → corruption may be accumulated-update-*count*, not size; needs a **2D N×C grid**) and **(b)** the spec's 2,000-*SIZE* regime + its exact sub-batch semantics are untested (may indict our `cache_c` algorithm, not compaction generally). The discriminating test is now the **2D N×C grid + a true-scale single-solve run** (gated on a larger stimulus pool), and verifying the spec's sub-batch semantics — not a single "N≥2,000" run.
...
| **Bounded incremental overlays + compaction** (the spec's runtime model) | **Yes — §8.3 Incremental Mode + §8.7 + §8.10** | **In-weight, viable WITH conditions (one untested)** | Per-commit overlays accumulate, bounded by §8.7 drift, reset by §8.10 compaction. **Conditions: (1) §8.7 concentration-aware (our `k≤1` amendment), not count-only; (2) compaction runs before the clean envelope is exceeded; (3) compaction's own (sub-batched) re-run stays clean — ⚠ D-D20-1: DIRECTIONALLY PRESSURED, not falsified (accumulating sub-batching reintroduces corruption even when the joint solve is clean; K-vs-C confound + 2,000-SIZE regime open).** This is where our D1/§8.7 + D20 work directly serves the spec. |
...
**F1 instruction:** Write F1's B3 position as **"in-weight is viable for the spec's compaction-bounded hybrid (§8.3/§8.7/§8.10), with THREE named conditions — one of which is not yet validated"** — NOT "batch-only ready," and NOT a blanket "recommend side-store." The conditions, all tracing to our own work: **(1)** the §8.7 drift trigger must be **concentration-aware** (the D-D1-2 `k≤1` + global-volume amendment), not the spec's count-only 1,500/8,000 — author this as a concrete §8.7 spec amendment; **(2)** compaction (§8.10) must fire before the clean envelope is exceeded for the project's actual write rate; **(3)** **compaction's own sub-batched re-run must stay clean at the realized drift size — ⚠ D-D20-1 directionally PRESSURED it (not falsified; K-vs-C confound + 2,000-SIZE regime open); F1 must carry it as a live, partially-evidenced open question, not assume it clean.** Carry the side-store as the recommendation **only** for a *high-churn-online* profile beyond the spec's bounded-then-compacted overlays (candidate = gated side-store / Parametric-RAG; conditional on the unproven routing layer, §4). Re-open Axis A only if a hard mid-generation / zero-latency reason-over-fact requirement is ever confirmed. The §1.1 dimensions (auditability, delete/update governance, security, routing, cost) remain separate F1 sub-decisions. **Operator input = a calibration, not a fork: the realistic incremental write *rate* sets compaction cadence (condition 2) + whether the high-churn row applies. ⚠ Condition 3 (compaction-at-scale cleanliness, N≥2,000) is a NEW required experiment and should be run AT OR ABOVE the 7B numeric transfer in the next-arc — if it fails, condition 2 cannot save the verdict.**
...
> **⚠ Qualifier (R13, D-R13-1, 2026-06-25):** the Axis-A premise "in-weight gives forward native-knowing" holds **on the trained phrasing**, but for COUNTERFACTUAL edits over strong native priors, paraphrase-firing degrades sharply (trained-prompt 100% → mean ~22% across natural paraphrases; CORPUS/28). Usable (paraphrase-robust) in-weight knowledge for neutral/novel facts + generalization-aware recipes is UNTESTED — the open R5/native-knowing question. Until shown, do not assume in-weight serves *usable* forward reads beyond the trained phrasing.

> **Update (R5, D-R5-1, 2026-06-25):** the R13 qualifier RESOLVES two-sided. **INSERT path** (new project facts = novel + thin): in-weight gives **paraphrase-robust** usable native-knowing (16/16 all-hit) → weights DO serve usable forward reads for the realistic insert fact-class. **UPDATE path** (overwrite known facts — spec-required §8/§11.12): fragile vs pretraining (0/16 all-hit, reverts-to-true), partially rescued by paraphrase-diverse training (→65% mean); overwrite-of-a-prior-edit = unmeasured. → B3N stays scope-keyed: in-weight viable for the insert/batch core; updates may need the diverse recipe and/or side-store. CORPUS/29.
