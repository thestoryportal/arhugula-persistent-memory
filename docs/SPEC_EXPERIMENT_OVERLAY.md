...
| **§8.10 Compaction** (line 413) | Full MEMIT re-run on archived patches every ≤30 days / at drift trigger; **resets anchor to clean**; delta-composition rejected | A1 `D-A1-1`; **D20 `D-D20-1` (CORPUS/23)** | Compaction = a batch rebuild → A1 clean at N≤100 single-batch. **D20 EVIDENCE:** the engine does a single joint solve (no real sub-batching — the 2,000 cap is a *spec prescription*, line 384); the spec's prescribed sub-batching **reintroduces corruption, robust by ~10 sub-batches** (−19 to −34pp). So **"returns to clean" FAILS once compaction sub-batches** (chunking component). *Open:* single-solve cleanliness at true scale + where the 2,000-SIZE boundary sits vs the C=10 floor (component 1; connects to §10.4). | **CHUNKING = EVIDENCED-FAILING (D20, weak-form)**; **scale component (single-solve at N≫100) = OPEN** |
