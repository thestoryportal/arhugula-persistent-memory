...
| **§8.10 Compaction** (line 413) | Full MEMIT re-run on archived patches every ≤30 days / at drift trigger; **resets anchor to clean**; delta-composition rejected | A1 `D-A1-1` (a batch compile = a compaction); **⚠ compaction-at-scale = OPEN (B3N condition 3)** | Compaction = a batch rebuild → A1 showed clean **only at N≤100/3B single-batch**. ⚠ **"Returns to clean" is UNVALIDATED at the realized drift size:** §8.7 allows 1,500–8,000 edges before compaction, and MEMIT batch size 2,000 (§10.4 / line 384) means compaction is **sub-batched = sequential solves** → the G6.1/D-D1-2 corruption mechanism. **Compaction at scale may reintroduce corruption.** *(Connects directly to the §10.4 row below — these were previously split.)* B3N condition 3. | **VALIDATED only at N≤100**; **compaction-at-scale (N≥2,000) = OPEN — the sharpest B3N falsifier** |
