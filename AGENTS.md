...
- **`advisor()` / advisor-review** → for **Codex-led work**, use **Claude as the out-of-family advisor** via `tools/claude_advisor.sh` (local Claude Code `claude.ai` Max subscription auth, no API key). Feed the finding/design + evidence + exact numbers; it does NOT see your transcript. Codex/GPT review is fallback or secondary for Codex-led work. For Claude-led work, use GPT-family Codex review for independence. Input, not authority; evidence binds.
...
- **`--mode agent`** appends an advisory independent review per reviewable staged finding. Default backend is Claude via `tools/claude_advisor.sh` for Codex-led work (`--advisor claude`; claude.ai Max subscription auth, no API key); `--advisor codex` remains fallback/secondary. Default `--mode batch` runs the unit command directly.
