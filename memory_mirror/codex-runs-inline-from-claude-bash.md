...
**⚠ Auth can EXPIRE mid-session + a working fallback (2026-06-25):** the ChatGPT-OAuth token died partway through a multi-experiment session with `refresh_token_reused` / `token_expired` (401, "refresh token already used") — codex then fails every call until the operator re-runs `codex login` (interactive; suggest `! codex login`). **Working cross-family fallback when codex is down: `mcp__perplexity__perplexity_reason` (Sonar Reasoning Pro)** — a genuinely different model family from Opus advisor; used it for R2/R13/R5 reviews and it caught real, load-bearing errors (metric-mismatch, identification confound, insert-vs-update mapping). Pass the full evidence inline (`strip_thinking:true`); it's not repo-aware. **Calibrate cross-family by result type ([[review-diminishing-returns-evidence-is-binding]]):** WAIVE it for decisive/foregone/mechanistically-forced results (R2 0/24, R5b p<1e-6) with a noted reason; RUN it where the interpretation is subtle (metric choice, identification, over-claim) — that's where an independent family adds signal.
