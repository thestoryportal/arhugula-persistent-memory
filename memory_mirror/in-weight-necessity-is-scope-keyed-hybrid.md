...
**VERDICT (SPEC-GROUNDED — the spec settles the write profile, §8.3/§8.4/§8.7/§8.10):** the spec's deployment model is a **COMPACTION-BOUNDED HYBRID** — batch genesis + bounded incremental per-commit overlays (`incremental_NNNN.vindex`) + periodic compaction (full MEMIT re-run resetting the anchor), NOT batch-only. **In-weight is viable for that hybrid under TWO conditions, both from our own work:** (1) the §8.7 drift trigger must be **concentration-aware** (our D-D1-2 `k≤1` amendment), NOT the spec's count-only 1500/8000; (2) compaction must fire before the clean envelope is exceeded. The side-store is preferred **only** for high-churn-online editing *beyond* the spec's bounded overlays. → **Our D1/§8.7 work is the linchpin that keeps the spec's incremental path safe.** (Earlier framing "batch-only / open operator fork" was WRONG — the operator redirected to the spec, primary source; advisor had under-weighted the spec text.)
