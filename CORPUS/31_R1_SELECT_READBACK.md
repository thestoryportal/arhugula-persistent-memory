...
- **Spec mechanism (CORRECTED 2026-06-25, D-C5-1 spec-read):** the spec does **not** rely on per-read divergence detection at all — its consistency model (D43 strong-consistency single-writer FIFO + mount-block; 2PC commit; **§11.14/C-OC3 compaction regression verify, CORE=1.0 abort**) is *designed* to keep the served store from silently diverging. So the in-weight per-read proxy R1 built was both **bleed-unsound AND not the spec's mechanism**. A 2PC commit-bit (R1-bit) covers commit-time; C-OC3 covers compaction-time. ⚠ The genuine residual gap is narrower: **D43's global 'no stale-read' vs C-OC3's tolerated ≤5% SUPPORTING / ≤20% INCIDENTAL post-compaction divergence + the tier-blind commit-bit** → a committed non-CORE fact can read committed yet be stale (the D-C5-1 audit, `docs/C5_COMPACTION_VERIFY_AUDIT.md`).
...
- Candidate follow-ups (operator-steer, NOT auto-run): (i) replace the logprob proxy with a **direct ledger commit-status bit** sourced from the G1 2PC ack — bleed-immune, but **explicitly only commit-time**, divergence-detection left open and flagged; (ii) audit the **spec-mandated** post-compaction verifier (§11.14/C-OC3, found via the D-C5-1 spec-read 2026-06-25 — the spec already specifies a CompactionProbeReport with CORE=1.0 abort; the open work is its soundness/power/scale, not building a detector).
