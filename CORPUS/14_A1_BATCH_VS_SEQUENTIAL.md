...
## Decision
**D-A1-1 ⟨D-A1-1@92b78833⟩:** batch (single joint solve) ACCEPTED as the Genesis-path write mode — cross-entity-clean at N≤100 where sequential is not. Does NOT close Track A: A2 sentinels remain the runtime-incremental fix. Fork (§8 A1 PASS branch): fold batch in → still run A2.
