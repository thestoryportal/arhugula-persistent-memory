...
This delivers the §8.9 L1 read-back contract **for commit time**: SELECT confirms exactly what the 2PC actually committed. It does **NOT** detect **post-commit divergence** — drift between the ledger and the served store from compaction / quantization / corruption (§11.3/D43/R10). That obligation is **DEFERRED to C1 (operator-scope-gated), NOT solved here.** → R1's status is now **"L1 read-back delivered for commit-time; post-commit divergence open,"** *not* "R1 delivered."
...
- **Sharpens the live frontier:** the remaining R1-class open work is the genuinely-hard **post-commit divergence detector** (C1 / R10), which a commit-time bit cannot cover. Feeds D-B3N-1 (the structured-read/governance layer is index/ledger-owned).
