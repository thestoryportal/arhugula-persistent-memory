...
### 3.8.9.1 S2.8 Empirical NV Record (v2.4.1 amendment)

P-7 cache-dispatch path validated empirically at S2.8 Stage 1 SECT execution (2026-05-01) on `meta-llama/Llama-3.1-8B` FP16, RTX 4090 24 GiB, RunPod NV mfs.

**Empirical anchors:**

| Surface | Result | Anchor |
|---|---|---|
| Cell 3 R1.3 cache-dispatch smoke-load (single-layer, placeholder model) | PASS in 2.72s; "Loading cached" log line confirmed for `model.layers.4.mlp.down_proj_float32_mom2_t100_100000.npz` | `/workspace/architecture_profile/stage_1_cache_state.json` field `r1_3_smoke_load.status` |
| Cell 9 Trial 0 (full fidelity; cfb-001 replicate 1) | PASS; 4 layers cache-hit logged ("Loading cached" lines for layers 5–8); 32.4s edit wall-time (cold); no fallback compute | `/workspace/stage_1_sect/trials/stage_1_trial_cfb-001_r1.json` |
| Cell 9 Trials 1–8 (8 additional trials across cfb-001/002/003 × replicates) | PASS; ~15s edit wall-time (warm path via MEMIT process-local matrix retention); no fallback compute | `/workspace/stage_1_sect/trials/stage_1_trial_*.json` |
| Cumulative cache-dispatch trials | 9/9 PASS — zero "Computing Cov locally" log lines across the entire trial loop | `/workspace/stage_1_sect/trial_loop_log.json` |
| Unmount band (IC-S23-4 anchor; intra-pod drift) | 9/9 PASS at \|drift\| = 0.00e+00 (bit-identical) — Block 2 HC-2 demonstration on GPT-J 6B reproduced on Llama-3.1-8B | `/workspace/stage_1_sect/aggregate_verdict.json` field `aggregate_pass.unmount` |
| Inter-trial baseline drift gate | 9/9 PASS at 0.00e+00 — model state pristine across trial boundaries via Copy-Unmount | per-trial JSONs field `intertrial_gate` |

**P-7 marker verification points exercised:**

| Verification surface | Anchor | S2.8 result |
|---|---|---|
| Cell 2 in-kernel grep (post-restart Jupyter kernel; same kernel that runs Cell 9) | Both v2.4 §3.8.8 markers present in `memit_main.py`: `mom2_batch_tokens = 100 if model.config.max_position_embeddings > 8192 else None` (dispatch line) + `batch_tokens=mom2_batch_tokens,` (kwarg presence) | PASS |
| Operator-side pod-terminal grep (Gate 1 at session entry) | Marker A line 262, Marker B line 272 of `memit_main.py` | PASS |
| Runtime cache-resolution (Cell 3 R1.3 + Cell 9 trial loop) | `apply_memit_to_model` invocation against post-P-6 cache files with `_t100_` segment resolves to a cache hit; no `causal_mask` OOM; no fallback compute | PASS |

**P-7 architectural-threshold dispatch behavior empirically confirmed:**

```
Llama-3.1-8B  config.max_position_embeddings = 131072 > 8192
              ↓
              mom2_batch_tokens = 100  (truncation branch)
              ↓
              layer_stats(..., batch_tokens=100, ...)
              ↓
              Cache filename suffix: _t100_100000.npz
              ↓
              Cache hit on NV-resident file produced under same suffix
              ↓
              Edit-time get_cov returns covariance matrix in 2.72s (R1.3)
                                                              ~15s (warm trial)
                                                              ~32s (cold trial)
              ↓
              Layer update applied with updated mom2 matrix
              ↓
              No causal_mask allocation in untruncated regime
              ↓
              No 24 GiB GPU OOM
```

**C-S26-1 + C-S26-2 ratification status:**

C-S26-1 (`batch_tokens=100` operationally mandatory for LLaMA-family npos > 8192 on 24 GiB-class GPUs) and C-S26-2 (cache-dispatch path explicit `batch_tokens` propagation) are both **EMPIRICALLY-CONFIRMED at full Stage 1 fidelity** via the 9/9 cache-dispatch PASS verdict. P-7 closes both constraints by construction (architectural-threshold dispatch + kwarg-presence post-state marker); S2.8 provides the empirical anchor.

**Forward applicability:**

The S2.8 empirical record extends to any 8B-class LLaMA-family base model with `max_position_embeddings > 8192` on RTX 4090 24 GiB-class hardware. The architectural threshold (`> 8192`) preserves GPT-J backward compatibility per v2.4 §3.8.6 (GPT-J: npos=2048 → mom2_batch_tokens=None → default branch → pre-P-7 cache filename form preserved byte-for-byte; ZERO substrate-shift on GPT-J family).

**S2.8 record location (canonical):**

| Artifact | Path | Content |
|---|---|---|
| Per-trial verdicts | `/workspace/stage_1_sect/trials/stage_1_trial_<fact>_r<rep>.json` (×9) | Per-trial cache-dispatch wall-time + post-edit/post-unmount probe records |
| Trial loop log | `/workspace/stage_1_sect/trial_loop_log.json` | Consolidated 9-trial dispatch history |
| Aggregate verdict | `/workspace/stage_1_sect/aggregate_verdict.json` | Stage 1 verdict aggregation per IC-S24-4 + provisional bands |
| Cache state | `/workspace/architecture_profile/stage_1_cache_state.json` | Cell 3 R1.1/R1.2/R1.3 verdicts + per-file SHA prefixes |
| Patch state | `/workspace/architecture_profile/stage_1_patch_state.json` | Cell 2 P-1/P-2/P-4/P-5/P-6/P-7 marker verification |
| Reproducibility manifest | `/workspace/reproducibility_manifest.json` field `sessions["2.8"]` | Complete S2.8 environment fingerprint + verdict + bridge fix record |

**Discovery context recap (cross-reference):**

P-7 was authored at S2.7 (2026-05-01) from S2.6 Cell 9 Trial 0 forensic findings (cache-dispatch fallback at edit-time produced causal_mask OOM at 32 GiB on 24 GiB hardware). The IC-S26-1 draft form was specified at S2.6 close; resolved into canonical §3.8 entry at S2.7 with three open design questions closed (D-S27-1 hardcoded literal, D-S27-2 body-only modification, D-S27-3 universal-safe via architectural threshold). S2.8 provides the empirical NV record: the patch operates as designed under full Stage 1 trial loop conditions on the Workstream 1 production target model.

---

### 3.8.9.2 Cross-Reference: C-S26-3 Dry-Run Gate Methodology Empirical Validation

S2.8 first-execution surface validated the C-S26-3 dry-run gate methodology authored at S2.7 (v2.4 §10.1). Specifically:

| C-S26-3 design intent | S2.8 empirical outcome |
|---|---|
| "Catch source-level verification defects before consumer-session execution" | Cell 2 line 59 hidden_size assertion failed first execution; bridge-fixed in-session (D-S28-1); routed to v1.4 hardening pass via OQ-S28-3 |
| "Surface architectural defects via lightweight smoke equivalent" | OQ-S27-1 deferred resolution: R1.3 (single-layer cache-dispatch smoke-load) caught the same defect class that full Cell 9 trial would catch, at ~1100–1500× lower cost. Recommendation: codify R1.3 in v1.4 §1.5 as canonical execution-dry-run gate |
| "Runbook hardening passes that do not exercise target cells against fully-patched live state cannot be ratified" | v1.3 textual ratification status: DEFECTIVE-IN-FIELD per strict criterion (line 59 marker mis-specification survived purely-textual review at v1.3 authoring). Methodology validated as designed |

**Implication for v1.4 +:** The C-S26-3 dry-run gate is now empirically anchored. Future hardening passes targeting cache-dispatch or other architectural surfaces should adopt R1.3-equivalent smoke-load tests as the canonical execution-dry-run form, with full trial loop execution reserved for Stage 1+ verdict generation.
...
*End of MEMIT Patches Canonical Specification v2.4.*
