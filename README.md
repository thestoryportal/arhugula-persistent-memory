...
**📍 PROGRAM STATE (updated 2026-06-26 (C10 multi-token FALSIFIER FIRED + c10c diagnosis; AnyEdit-port = operator-gated multi-day choice, D-C10-1/D-C10b-diverse))** — _auto-generated from `docs/program_state.json` — DO NOT edit between the markers; run `python3 tools/render_state.py --write`._
...
- **Next actions (priority):**
  ...
  2. (1) C10 — OPERATOR-GATED choices (do not roll in): (a) AnyEdit port (multi-day; mechanistic case, payoff unproven; PORT jianghoucheng/AnyEdit) to fix arbitrary multi-token continuation; OR (b) empirical residual-test with project-coined multi-word SEMANTIC values to size the real in-weight exposure; OR (c) accept C10 as the documented bounded limitation. §7.1 removes syntactic values (Git's job); the semantic residual is use-case-dependent + plausibly significant.
