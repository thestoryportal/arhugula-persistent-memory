...
**📍 PROGRAM STATE (updated 2026-06-26 (C10 band-knob done — later [8,12] NO MATERIAL RESCUE; D-C10e-bandknob/CORPUS/37; wider/strength knobs still untested))** — _auto-generated from `docs/program_state.json` — DO NOT edit between the markers; run `python3 tools/render_state.py --write`._
...
- **Next actions (priority):**
  ...
  2. (1) C10 — RESIDUAL FRAGILE plus BAND-KNOB NO MATERIAL RESCUE (D-C10b-residual/CORPUS/36; D-C10e-bandknob ⟨D-C10e-bandknob@82b491dc⟩/CORPUS/37): realistic project-coined multi-word SEMANTIC values remain the on-target blocker. Later `[8,12]` alone fails: A7 para_full 13.9 to 5.6 while canon_full only 29.2 to 45.8, so pure later-band W-realization is not enough. Remaining OPERATOR-GATED choices: (A) AnyEdit port (multi-day; mechanistically consistent, not proven necessary; PORT jianghoucheng/AnyEdit), (B) one more cheap W-realization knob before the port — widened `[4,12]` or edit-strength/layer-count on A7, (C) accept-as-bounded, viable only if CORE in-weight values are single-token / prior-coherent. Residual-test and pure later-band test are COMPLETE.
