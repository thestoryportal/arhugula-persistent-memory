...
**📍 PROGRAM STATE (updated 2026-06-26 (C10 RESIDUAL-TEST done — RESIDUAL FRAGILE; D-C10b-residual/CORPUS/36; AnyEdit option strengthened, still operator-gated))** — _auto-generated from `docs/program_state.json` — DO NOT edit between the markers; run `python3 tools/render_state.py --write`._
...
- **Next actions (priority):**
  ...
  2. (1) C10 — RESIDUAL-TEST DONE (D-C10b-residual, CORPUS/36): RESIDUAL FRAGILE — realistic project-coined multi-word SEMANTIC values 19–31% para-full (≪85% gate). Remaining OPERATOR-GATED choices: (A) AnyEdit port (multi-day; z-probe shows the bottleneck is W-realization not compute_z (z=0.99 achievable) → AnyEdit is mechanistically CONSISTENT but NOT proven necessary; the cheaper W-realization knobs (band width/layer count/edit strength) are UNTESTED — re-run one realistic arm with a wider/later band BEFORE the multi-day port; payoff unproven; PORT jianghoucheng/AnyEdit) — strengthened over (C) accept-as-bounded (viable only if a project's CORE in-weight values are single-token / prior-coherent). Option (b) residual-test is now COMPLETE.
