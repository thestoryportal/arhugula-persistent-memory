...
| **🚦 Close-out gate (`tools/closeout_check.py`)** | **MANDATORY** as the LAST step of every experiment close, before declaring done / writing `SESSION_CHECKPOINT.md`. `python3 tools/closeout_check.py <D-ID>` → must be ✅ ALL GREEN. | It is the mechanical WRITE-side gate (DISCIPLINE §1.1); a RED gate means the experiment is **not done**. Don't hand a close-out to the operator to verify — the script verifies. Extend `REQUIRED` in the script when a new canonical tracker is added. |
