...
- **After EVERY experiment** (the `EXPERIMENT_RUNBOOK.md` §0.4 close-out) — the **COMPLETE** canonical-tracker set (this list was under-specified before 2026-06-21): write `CORPUS/NN_*.md` → update `CORPUS/00_MASTER_EVIDENCE.md` + `CORPUS/03_STATUS_LEDGER.md` + `CORPUS/01_PROVENANCE_MANIFEST.md` + `CORPUS/02_VANDV_CHAIN.md` → runbook **§0.3 + §12 + §13** (+ §5 Decision-ID) → `PROGRESS.md` (move the node ①/③/④ + ⑤ scorecard) → `docs/HYPOTHESIS_REGISTER_*.md` → `EVIDENCE_INDEX.md` + `docs/EXPERIMENT_REGISTRY.md` → `SESSION_CHECKPOINT.md` → memory + `memory_mirror/` (+ `MEMORY.md` index).
