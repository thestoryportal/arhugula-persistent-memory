...
def compute_P():
    ...
    for layer in L:
        ...
        U,S,_=torch.linalg.svd(cov, full_matrices=False)  # GPU SVD (3B band-search; cov fits beside the 3B model)
...
def my_edit(requests, mode, P=None, cache_c=None):
    ...
    for i,layer in enumerate(L):
        ...
        if mode=="memit":
            ...
            Pi=P[i].cuda(); ca=cache_c[i].cuda(); Kg=Kd.float().cuda(); rg=rd.float().cuda()   # GPU solve (3B 11008-dim fits)
...
def heldout_top1(post):  # top-1 stability (vs pre) + correctness (vs truth), grouped by relation class
    ...
    for grp,rels in [("edited_rel",["capital","language"]),("continent",["continent"]),("all",["capital","language","continent"])]:
        ...
        out[grp]={"top1_stable_vs_pre":round(100*sum(post[h["prompt"]]["id"]==pre[h["prompt"]]["id"] for h in sub)/len(sub),1),
                  ...
                  "top1_exact_vs_truth":round(100*sum(post[h["prompt"]]["tok"].strip().lower()==h["truth"].strip().lower() for h in sub)/len(sub),1)}
    return out
