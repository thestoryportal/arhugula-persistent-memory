...
- **Latest:** D-D20-1 (compaction sub-batch granularity — B3N condition 3, component 2; CORPUS/23): the engine does a SINGLE joint solve regardless of N → the spec's 'sub-batch at 2,000' is a PRESCRIPTION, not engine behavior, and sub-batching = sequential accumulation = the corruption mechanism. WEAK-FORM FALSIFICATION (not a promoted node): partitioning a clean N=100 joint solve into accumulating sub-batches reintroduces held-out cross-entity corruption, ROBUST by C=10 (10 sub-batches: −19 to −34pp across 3 edit orderings; expr 96-100% excludes under-editing; anchor gates C=100→A1-clean & C=1→A0-corrupt PASS all 3). C=25 (4 sub-batches) is ORDER-SOFT (−3 to −19pp; within noise at one ordering) → NOT promotable; robust floor = C=10. BOUND: can falsify but NOT confirm condition 3 (N held at known-clean 100 → isolates chunking, not scale). SCALE component (single-solve cleanliness at true drift size; the spec's 2,000-SIZE boundary is 200× C=10) remains OPEN — gated on a larger stimulus pool (operator effort call). → B3N condition 3 now EVIDENCED-FAILING in its chunking component (was UNTESTED): the spec's sub-batched compaction cannot be assumed to return to clean. Dual-reviewed (advisor ×2 + gpt-5.5 at promote gate). docs/D20_COMPACTION_SUBBATCH_PREREG.md.
...
- ⭐ LATEST (2026-06-21): **D-D20-1** — **compaction sub-batch granularity = WEAK-FORM FALSIFICATION of B3N condition 3 (chunking component); CORPUS/23.** Engine finding: kmeng01/memit does a **single joint solve regardless of N** → the spec's "sub-batch at 2,000" (line 384) is a **PRESCRIPTION, not engine behavior**, and sub-batching = sequential accumulation = the corruption mechanism. **Result:** partitioning a clean N=100 joint solve into accumulating sub-batches reintroduces held-out cross-entity corruption, **ROBUST by C=10** (10 sub-batches: −19 to −34pp across 3 edit orderings; expr 96-100% excludes under-editing; anchor gates C=100→A1-clean & C=1→A0-corrupt PASS all 3). **C=25 (4 sub-batches) is ORDER-SOFT** (−3 to −19pp; within noise at one ordering) → **NOT promotable; robust floor = C=10.** → **B3N condition 3 now EVIDENCED-FAILING in its chunking component** (was UNTESTED): the spec's sub-batched compaction cannot be assumed to return to clean. **BOUND: falsifies but doesn't confirm** — N held at known-clean 100 isolates **chunking, not scale**; the **SCALE component (component 1)** — single-solve cleanliness at true drift size + where the spec's 2,000-SIZE boundary sits vs the C=10 floor (200× bigger) — remains **OPEN, gated on a larger screened stimulus pool (operator effort call).** Reasoned/empirical weak-form finding, **NOT a promoted node** (like C2-band). Dual-reviewed (advisor ×2 + gpt-5.5 promote-gate). Situated on the e2e map: **§8/§11.14 Write-Engine·Memory-lifecycle** cell. **Full detail: `CORPUS/23`, `docs/D20_COMPACTION_SUBBATCH_PREREG.md`.** **NEXT (per e2e coverage map, PROGRESS ⓪): the READ/QUERY CONTRACT (CP2) — the biggest empty cell — ahead of more write-engine scaling.**
...
- **2026-06-21 (D-D20-1 — compaction sub-batch granularity DONE = WEAK-FORM FALSIFICATION; `CORPUS/23`).** B3N condition 3, component 2. Engine finding: kmeng01/memit does a single joint solve regardless of N → the spec's 2,000-edit sub-batch cap (line 384) is a PRESCRIPTION, not engine behavior; sub-batching = sequential accumulation = corruption. Pre-reg `docs/D20_COMPACTION_SUBBATCH_PREREG.md` (advisor-checked); runner `experiments/track_d/d20_subbatch_sweep.py`; results `results/d20_subbatch_sweep_result{,_ord1,_ord2}.json`. Chunk-size sweep at fixed N=100, fixed-base P, accumulating cache_c, 3 edit orderings; C=N=A1, C=1=A0 by construction (anchor gates pass all 3). **Result:** held-out cross-entity top-1 falls as chunks shrink — **ROBUST corruption by C=10** (−19 to −34pp/3 orderings), **C=25 ORDER-SOFT** (−3 to −19pp, within noise at ord2) → robust floor = C=10, C=25 not promotable; expr 96-100% (under-editing excluded), baseline 100% (clean instrument). → **B3N condition 3 EVIDENCED-FAILING (chunking)**: sub-batched compaction can't be assumed clean. **BOUND: falsifies not confirms** (N held at known-clean 100 → isolates chunking not scale); **SCALE component OPEN** (single-solve at true drift size + the spec's 2,000-SIZE boundary vs the C=10 floor; gated on a larger stimulus pool — operator effort call). NOT a promoted node. Advisor ×2 (pre-auth + reconcile-mandated 2nd-ordering re-run that caught the C=25 over-claim) + gpt-5.5 promote-gate. e2e cell: §8/§11.14. **NEXT (per PROGRESS ⓪ coverage map): the READ/QUERY CONTRACT (CP2) — biggest empty cell — ahead of more write-engine scaling.**
