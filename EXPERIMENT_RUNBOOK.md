...
- **When to update:** (a) any experiment completes → fill its block's RESULT + resolve its FORK + update §0.3 + §12 dashboard + §13 changelog; (b) any decision (model/method/tool) → add/append §5 with a Decision-ID; (c) any new dependency → §6; (d) any durable learning → memory (§2.7) + a one-line §13 entry.
...
| **Batch (single joint solve) for foundational writes** | **ACCEPTED-for-Genesis (D-A1-1)** | A1: ELIMINATES cross-entity corruption (held-out FLAT 100% N≤100 vs seq 42%); Genesis(§7.7) is a batch. Doesn't fix incremental path. | If degrades past N=100. |
